    ...
    <div class="row add-todo-button-initial">
      <span class="col">
        <div :class="{'col':todos.length>0}">
            <input class="col-md-6 new-todo m-3 p-3" type="text" v-model="doTo" @keypress.enter="addNewTodo" placeholder="click and add new todo">
            ...
            <li class="m-2 p-2" v-for="(todo,index) in todos" :key="todo._id">
              <div class="col-8">
                <span  v-if="isEditMode && index === selectedEditItemIndex" class="col-md todos">
                  <input class="todo-edit-input" type="text" v-model="selectedEditItem" @keypress.enter="EditTodo(todo, index)" />
                </span>
              ...
              <div class="col-4">
                <span>
                  <button v-if="!isEditMode" :style="todo.completed ? {border:'3px solid green'} : {color:''}" class="col-xs m-2 p-2 del-button" @click="DeleteTodo(todo)">Delete</button>
